...
import { FieldValidationRule } from '../field-rules/base.field-v-rule';
import { NotSpecifiedDTOArrayAttrsValidationRule } from '../field-rules/default/not-specified-dto-array-attrs.field-v-rule';
import { NotSpecifiedDTOAttrsValidationRule } from '../field-rules/default/not-specified-dto-attrs.field-v-rule';
...
import {
  ...
  BaseTypeValidationConfig, FieldValidationStatus,
...
> extends BaseFieldValidator<R, AC, DTOC> {
  ...
  protected validateInternal<V extends ValidatableType>(value: V): FieldValidationStatus {
  ...
  private getDTOCheckRules(): FieldValidationRule[] {
    const dtoCheckRules: FieldValidationRule[] = [];

    if (this.config.arrayConfig.isArray) {
      dtoCheckRules.push(
        new NotSpecifiedDTOArrayAttrsValidationRule(
          Object.keys(
      ...
      dtoCheckRules.push(
        new NotSpecifiedDTOAttrsValidationRule(
          Object.keys(
            (this.config.typeConfig as unknown as DTOTypeValidationConfig<ValidatableDTO>)
              .validatorMap,
          ),
