...
describe('Validating strings using regular expressions', () => {
  test('Text must end with world', () => {
    const sut = new RegexReturnTrueFormatValidationRule(/world$/, 'Текст должен заканчиваться на слово world');
  ...
  test('success ИИН is valid', () => {
    const sut = new RegexReturnTrueFormatValidationRule(/^[0-9]\d{11}$/, 'Введите ИИН (12 цифр) ');
  ...
  test('success, correct phone number entered', () => {
    const sut = new RegexReturnTrueFormatValidationRule(/^\+\d{1,3} \d{3} \d{3} \d{4}$/, 'Введите номер в формате +7 702 777 7777');
  ...
  test('failure, incorrect phone number entered', () => {
    const sut = new RegexReturnTrueFormatValidationRule(/^\+\d{1,3} \d{3} \d{3} \d{2}$/, 'Введите номер в формате +7 702 777 7777');
